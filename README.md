...
## 🗂 Google Drive 與 GitHub 的分工（重要，別再搞混）

兩個都用，但做不同的事：

| | 角色 | 放什麼 / 怎麼用 |
|---|---|---|
| **Google Drive** | 你的**私人工作室** | Figma 匯出、原始圖、文案草稿、備份。愛怎麼丟怎麼丟，任何電腦都能開。**只有你看得到** |
| **GitHub** | 你的**公開櫥窗** | 只放「要給世界看的網站成品」。要更新網站時，把成品放上去（可請 Claude 幫忙）。**全世界看得到** |

**為什麼網站不能放 Drive？** Google 在 2016 年關掉了「用 Drive 當網站」的功能——
現在 Drive 只能當「檔案櫃」（給人下載檔案），不能當「店面」（把 HTML 變成活的網站）。
所以網站一定得放 GitHub，Drive 做不到這件事。

> 一句話：**在 Drive 工作 → 成品放 GitHub → 網站上線。**
